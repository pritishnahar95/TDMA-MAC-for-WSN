...
module TMsgBeaconC @safe()
{
	uses
	{
	    ...
	    interface AMSend as Send_Super_0;
	    interface AMSend as Send_Super_1;
	    interface AMSend as Send_Super_2;
	    interface AMSend as Send_Super_3;
	    interface AMSend as Send_Super_4;
	    interface AMSend as Send_Super_5;
	    interface AMSend as Send_Super_6;
	    interface AMSend as Send_Super_7;
	    interface AMSend as Send_Super_8;
	    interface AMSend as Send_Super_9;
	    interface AMSend as Send_Super_A;
	    interface AMSend as Send_Super_B;
	    interface AMSend as Send_Super_C;
	    interface AMSend as Send_Super_D;
	    interface AMSend as Send_Super_E;
	    interface AMSend as Send_Super_0_Data;
	    interface AMSend as Send_Super_1_Data;
	    interface AMSend as Send_Super_2_Data;
	    interface AMSend as Send_Super_3_Data;
	    interface AMSend as Send_Super_4_Data;
	    interface AMSend as Send_Super_5_Data;
	    interface AMSend as Send_Super_6_Data;
	    interface AMSend as Send_Super_7_Data;
	    interface AMSend as Send_Super_8_Data;
	    interface AMSend as Send_Super_9_Data;
	    interface AMSend as Send_Super_A_Data;
	    interface AMSend as Send_Super_B_Data;
	    interface AMSend as Send_Super_C_Data;
	    interface AMSend as Send_Super_D_Data;
	    interface AMSend as Send_Super_E_Data;
...
implementation
{
	...
	int head = 0;
	uint16_t new_node;
	uint8_t sub_head = 3*16 + 0;
	uint8_t super_head = 1*16 + 0;
	uint8_t new_sub_head = 3*16 + 0;
...
//Energy_cal_variables : 
	...
	uint16_t send_count_ts = 0;
	uint16_t send_count_ts_head = 0;
	uint16_t send_count_data = 0;
	uint16_t send_count_data_head = 0;
	
	uint16_t recv_count_ts = 0;
	uint16_t recv_count_ts_head = 0;
	uint16_t recv_count_data = 0;
	uint16_t recv_count_data_head = 0;
...
//for resetting energy_cal_counters
 	 ...
 	 event void AMControl.stopDone(error_t err) 
	{
  	...
  	void sleep(long int period)
  	{
  		...
  		int_send = (int)(100*energy_send);
  		int_recv = (int)(100*energy_rec);
  		int_dim = (int)(100*energy_dim);
...
//timers start
	...
	event void Timer_ts_head.fired()
	{
		...
		if(phase == 1)
		{
			...
			if(super_head == ((LVL-1)*16+0))
			{
				call Send_Super_0.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+1))
			{
				call Send_Super_1.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+2))
			{
				call Send_Super_2.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+3))
			{
				call Send_Super_3.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+4))
			{
				call Send_Super_4.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+5))
			{
				call Send_Super_5.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+6))
			{
				call Send_Super_6.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+7))
			{
				call Send_Super_7.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+8))
			{
				call Send_Super_8.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+9))
			{
				call Send_Super_9.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+10))
			{
				call Send_Super_A.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+11))
			{
				call Send_Super_B.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+12))
			{
				call Send_Super_C.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+13))
			{
				call Send_Super_D.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
			else if(super_head == ((LVL-1)*16+14))
			{
				call Send_Super_E.send(AM_BROADCAST_ADDR, &pckt_super, sizeof(ts_msg_head_t));
			}
	...
	event void Timer_data.fired()
	{
		...
		if(head == 0)
		{
			...
			else
			{
				...
				energy_send += 4 * 8 * ival_send * voltage / data_rate;
		...
		else
		{
			...
			else
			{
				...
				energy_send += 8 * voltage * ival_send_head / data_rate ;
				energy_dim += 8 * voltage * ival_send_head / data_rate ;
				
			}
			call CC2420Packet.setPower(&dpckt_super, 23);
			//call Send_Super_0_Data.send(AM_BROADCAST_ADDR, &dpckt_super, sizeof(data_head_t));
			
			if(super_head == ((LVL-1)*16+0))
			{
				call Send_Super_0_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+1))
			{
				call Send_Super_1_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+2))
			{
				call Send_Super_2_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+3))
			{
				call Send_Super_3_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+4))
			{
				call Send_Super_4_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+5))
			{
				call Send_Super_5_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+6))
			{
				call Send_Super_6_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+7))
			{
				call Send_Super_7_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+8))
			{
				call Send_Super_8_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+9))
			{
				call Send_Super_9_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+10))
			{
				call Send_Super_A_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+11))
			{
				call Send_Super_B_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+12))
			{
				call Send_Super_C_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+13))
			{
				call Send_Super_D_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
			else if(super_head == ((LVL-1)*16+14))
			{
				call Send_Super_E_Data.send(AM_BROADCAST_ADDR, &dpckt_super, length);
			}
...
//receives start
	...
	event message_t* TSReceive_Head.receive(message_t* bufPtr, void* payload, uint8_t len)
	{
		...
		else if(phase == 4)
		{
			...
			phase = 0;
			current = 1;
			call Timer_ts.startPeriodic(timings - 10);
		}
	...
	event message_t* DataReceive.receive(message_t* bufPtr, void* payload, uint8_t len)
	{
		...
		else
		{
			...
			energy_rec += voltage * ival_rec * 8 * 4 / data_rate ;
	...
	event message_t* DataReceive_Head.receive(message_t* bufPtr, void* payload, uint8_t len)
	{
		...
		if(len == 11)
		{
			energy_rec += 1 * 8 * voltage * ival_rec / data_rate ; 
...
//sends start
	...
	event void Send_Super_0.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_1.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_2.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_3.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_4.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_5.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_6.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_7.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_8.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_9.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_A.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_B.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_C.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_D.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_E.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_ts_head++;
	}
	event void Send_Super_0_Data.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_data_head++;
	}
	event void Send_Super_1_Data.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_data_head++;
	}
	event void Send_Super_2_Data.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_data_head++;
	}
	event void Send_Super_3_Data.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_data_head++;
	}
	event void Send_Super_4_Data.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_data_head++;
	}
	event void Send_Super_5_Data.sendDone(message_t* bufPtr, error_t error)
	{
		// send_count_data_head++;
	}
	event void Send_Super_6_Data.sendDone(message_t* bufPtr, error_t error)
	{
		// send_count_data_head++;
	}
	event void Send_Super_7_Data.sendDone(message_t* bufPtr, error_t error)
	{
		// send_count_data_head++;
	}
	event void Send_Super_8_Data.sendDone(message_t* bufPtr, error_t error)
	{
		// send_count_data_head++;
	}
	event void Send_Super_9_Data.sendDone(message_t* bufPtr, error_t error)
	{
		// send_count_data_head++;
	}
	event void Send_Super_A_Data.sendDone(message_t* bufPtr, error_t error)
	{
		// send_count_data_head++;
	}
	event void Send_Super_B_Data.sendDone(message_t* bufPtr, error_t error)
	{
		// send_count_data_head++;
	}
	event void Send_Super_C_Data.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_data_head++;
	}
	event void Send_Super_D_Data.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_data_head++;
	}
	event void Send_Super_E_Data.sendDone(message_t* bufPtr, error_t error)
	{
		//send_count_data_head++;
	}
